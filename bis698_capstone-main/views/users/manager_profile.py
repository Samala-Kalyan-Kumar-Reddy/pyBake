...
class ManagerProfile():
    ...
    def __init__(self,parent,user_obj,manager_dashboard):
        ...
        tk.Label(self.user_status_frame, text="Role:", font = ('helvetica', 11,"bold")).grid(row = 0, column = 0,sticky="w")
        tk.Label(self.user_status_frame, text=user_obj['role'],font = ('helvetica', 11)).grid(row = 0, column = 1,sticky='w')

        tk.Label(self.user_status_frame, text="Status:", font = ('helvetica', 11,"bold")).grid(row = 1, column = 0,sticky="w")
        ...
        tk.Label(self.user_status_frame, text="Date Joined:", font = ('helvetica', 11,"bold")).grid(row = 2, column = 0,sticky='w')
        ...
        tk.Label(self.user_status_frame, text="Logged In:", font = ('helvetica', 11,"bold")).grid(row = 3, column = 0,sticky='w')
       ...
       #PERMISSIONS
        ...
        tk.Label(self.permissions_frame, text="* Create User Account",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 1, column = 0,pady =5,padx =20,sticky = "w")
        tk.Label(self.permissions_frame, text="* Reset User Password",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 2, column = 0,padx =20,sticky = "w")
        tk.Label(self.permissions_frame, text="* Act/Deactivate User Account",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 3, column = 0,pady = 5,padx =20,sticky = "w")
        

        #PERMISSIONS
        tk.Label(self.product_frame,text = "",font = ('helvetica', 15,'bold'),bg = "#EEEEEE", fg = "#263238").grid(row = 0, column = 0,padx =20)
        tk.Label(self.product_frame, text="* Create Products",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 1, column = 0,padx =20,pady =5,sticky = "W")
        tk.Label(self.product_frame, text="* Edit and Delete Products",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 2, column = 0,padx =20,sticky = "W")
        tk.Label(self.product_frame, text="* Generate Product Reports",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 3, column = 0,padx =20,pady = 5,sticky = "W")
        


        tk.Label(self.reporting_frame, text="",font = ('helvetica', 15,'bold'),bg = "#EEEEEE", fg = "#263238").grid(row = 0, column = 0,padx =20)
        tk.Label(self.reporting_frame, text="* Create Customers",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 1, column = 0,padx =20,pady =5,sticky = "W")
        tk.Label(self.reporting_frame, text="* Generate Customer Reports",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 2, column = 0,padx =20,sticky = "W")
        tk.Label(self.reporting_frame, text="* Generate Users Report",font = ('helvetica', 10),bg = "#EEEEEE", fg = "#263238").grid(row = 3, column = 0,padx =20,pady = 5,sticky = "W")
    ...
    #     #TOP BANNER FRAME
    #     self.top_banner_frame = tk.Frame(self.parent,height = 40, bg="gray")
    #     self.top_banner_frame.grid(row = 0, column=4,columnspan=2,sticky="ew")


    #     #PICTURE FRAME
    #     self.picture_frame = tk.Frame(self.parent,height = 200,width = 200,bg="red")
    #     self.picture_frame.grid(row = 1, column=1, sticky="E")


    #     self.tk_image = Utils.resize_image((100,100),'images/user.png')
    #     # Create a Label widget with the resized image
    #     self.user_image = tk.Label(self.picture_frame, image=self.tk_image)
    #     self.user_image.grid(row = 0, column =0,sticky="WE")

    #     self.user_label = tk.Label(self.parent, text="User Profile",font= ('Helvetica',18),foreground ='#008fd3')
    #     self.user_label.grid(row = 0, column = 0)

    #     tk.Label(self.picture_frame, text=f"{user_obj['firstname'].capitalize()} {user_obj['lastname'].capitalize()}",font = ('helvetica', 15)).grid(row = 1, column = 0,sticky='E')
    #     tk.Label(self.picture_frame, text=f"Status: {user_obj['status']}",font = ('helvetica', 9)).grid(row = 2, column = 0,sticky='E')
    ...
    #     self.password_image.grid(row = 3, column =1,pady = 20, padx = 10,sticky="E")

    #     self.change_password_label = tk.Label(parent, text="Change Password",font = ('helvetica', 10),cursor = "hand2")
    #     self.change_password_label.grid(row = 3, column =2, sticky='E')
    #     self.change_password_label.bind("<Button-1>",self.on_change_password_click)
    ...
    #     self.logout_image.grid(row = 3, column =3,sticky="E")
    #     self.logout_label = tk.Label(parent, text="Logout",font = ('helvetica', 10) ,cursor = "hand2")
    #     self.logout_label.grid(row = 3, column =2 ,padx = 10,sticky='E')
    #     self.logout_label.bind("<Button-1>",self.on_logout_label_click)
    #     tk.Button(parent,text="Logout", command=self.logout).grid(row =10,column =2,sticky='SE')
